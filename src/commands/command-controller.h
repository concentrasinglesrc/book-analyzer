...
public:
  ...
  int add(AddOrderDetails const order_details, long const target_shares) {
    log.debug(
        "AddOrder {id : %s, timestamp: %ld, side: %s, price: %.2f, shares: %d}",
        order_details.get_id().c_str(), order_details.get_timestamp(),
        order_details.get_side().c_str(), order_details.get_price(),
...
public:
  ReduceOrderCommand(DataHandler &handler, Serializer &serializer,
                     Viewer &viewer)
...
public:
  ...
  std::unique_ptr<Command> parse_command(std::string const &command,
    ...
    if (dispatcher.end() == it)
      return std::unique_ptr<Command>();
    else
      return it->second(handler, serializer, viewer);
  }
};

logger::Log AddOrderCommand::log(__FILE__ "-AddOrderdCommand", logger::INFO);
logger::Log ReduceOrderCommand::log(__FILE__ "-ReduceOrderdCommand",
                                    logger::INFO);

...
namespace book
{
    class Analyzer {
        ...
        int run(long target_shares) {
            ...
            do {
                ...
                rc = page.read([=](std::string const & line){
                    ...
                    if (!validator->validate_order(args)) return -1;
                    auto command = controller->parse_command(args[1], *serializer, *handler, *viewer);
                    if (command) command->exec(args, target_shares);
                    else log.warn("malformed command"); 
        ...
        void on_init(void) {
          ...
          controller->subscribe("R", std::bind(Command::make_command<ReduceOrderCommand>, 
              std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
